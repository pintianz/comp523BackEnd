...
$(document).ready(function () {
    ...
    $.ajax(baseUrl + "adminLogin.php", {
        ...
        success: function (data, status, xhr) {
            ...
            //Get Admin User information and store user object
            var inputData = {userID: data.userID};
            $.ajax(baseUrl + "users.php", {
                type: "POST",
                dataType: "json",
                ...
                success: function (r, status, xhr) {
                    if (r) {
                        admin = r;
                        $("span.userName").html(r.name);
                        $("#userEmail").html(r.email);
                    }
                }
            });

            inputData = {action: "getOneByID", siteID: data.siteID};
            $.ajax(baseUrl + "sites.php", {
                async: false,
                type: "POST",
                dataType: "json",
                data: JSON.stringify(inputData),
                async: false,
                        success: function (r, status, xhr) {
                            if (r) {
                                site = r;
                                $("span.siteName").html(r.siteName);
                            }
                        }
            });
            //by default sort by surveyID
            $("#surveyID").css("text-decoration", "underline");
            downloadSurveys();
        },
    ...
    $("#signout").click(function (e) {
        $.ajax({
            url: baseUrl + "adminLogin.php?signout=1",
            type: "GET",
    ...
    $("#pwButton").click(function (e) {
        ...
        var $form = $("div.changeSitePw form");

        if (!userPw || !newSitePw || !confirmPw) {
            $(".error").remove();
            $form.before("<p class='error'>Please fill all three fields!</p>");
            return;
        }
...
function downloadSurveys() {
    ...
    var inputData = {action: "getAllBySiteID",
        siteID: site.siteID,
...
function loadSurvey(survey, isOdd, download) {
    ...
    var image = "<a href = '../" + survey.leavePhoto + "' target = '_blank'><img src='../" + survey.leavePhoto + "' onerror='hideSurveyPic(" + survey.surveyID + ")'></a>";
    $tr.append("<td>+</td><td>" + survey.surveyID + "</td><td class='userID'>" + survey.userID + "</td><td>"  + survey.circle + "</td><td>" + survey.survey + "</td><td>" + survey.timeStart + "</td><td>" + survey.temperatureMin + " - " + survey.temperatureMax + "</td><td>" + survey.plantSpecies + "</td><td>" + survey.herbivory + "</td><td>" + image + "</td><td><button class='deleteSurvey'>Delete Entry</button></td><td><button class = 'markUserInvalid'>Mark User as Invalid</button></td>");
    $tbody.append($tr);
    if (download) {
...
function loadOrder(order) {
    ...
    var image = "<a href = '../" + order.insectPhoto + "' target = '_blank'><img src='../" + order.insectPhoto + "' onerror='hideOrderPic(" + order.orderID + ")'></a>";
